...
| 평가 요소 | 배점 | 본 문서 반영 장 |
|---|---|---|
| 전체 데이터 흐름 표현 — 클라이언트-서버-DB-외부 LLM API 간 흐름 | 5 | 3장 |
| 배포 아키텍처 표현 — Nginx-Gunicorn-Django(WSGI) 연계 구조 | 5 | 4장 |
| 클라우드/컨테이너 구성 표현 — AWS 리소스와 Docker 컨테이너·네트워크 관계 | 5 | 5장 |
| 보안/확장성 고려 — 보안 그룹, 권한 분리 등 | 5 | 6장 |
...
| 구성 요소 | 역할 | 실행 방식 | 포트 |
|---|---|---|---|
| React (Vite 빌드) | 화면 라우팅, 반응형 UI, 입력/결과/마이페이지/관리자 화면 | Nginx 컨테이너에 정적 파일로 서빙 | - |
| Nginx | 리버스 프록시, HTTPS 종료, 정적 파일 서빙, 요청 경로 라우팅 | Docker 컨테이너, 유일하게 호스트에 포트 퍼블리시 | 80, 443 |
| Django (Gunicorn) | 세션 인증, 회원가입/로그인/로그아웃, 마이페이지, 작명 이력 CRUD | Docker 컨테이너, Compose 내부 네트워크 전용 | 8000 |
