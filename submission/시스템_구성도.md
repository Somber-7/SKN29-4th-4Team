...
| cron | `webapp/Dockerfile` 재사용, `aggregate_daily_stats` 반복 실행 셸 커맨드로 진입점 재정의 | 없음 | - (배치 전용, 수신 포트 없음) | `unless-stopped` |
| fastapi | `deploy/Dockerfile.fastapi` (Uvicorn) | 없음 | 8001 | `unless-stopped` |
| neo4j | `neo4j:5` 공식 이미지 | 없음 | 7474, 7687 | `unless-stopped` |

`cron` 컨테이너는 django 이미지를 그대로 재사용해 기동 시 즉시 1회 + 이후 24시간마다 `python manage.py aggregate_daily_stats`를 실행한다. 관리자 대시보드의 "오늘" 통계·주간 추이가 항상 0으로 나오던 문제(이 배치가 어디서도 스케줄링되지 않았던 것이 원인)를 해결하기 위해 추가되었다.
...
| 대상 | 인바운드 허용 | 근거 |
|---|---|---|
| EC2 보안 그룹 | 22(SSH), 80(HTTP), 443(HTTPS) — Django(8000)·FastAPI(8001)·Neo4j(7474/7687)는 인바운드 규칙 자체가 없음 | `assets/시스템_아키텍처.svg`, `docs/EC2_배포_매뉴얼.md` Part A-0 |
| RDS 보안 그룹 | 5432 포트, 소스는 EC2 보안 그룹으로 한정(퍼블릭 액세스 비활성) | `docs/EC2_배포_매뉴얼.md` Part A-4 |
